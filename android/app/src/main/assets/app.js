...
// ==========================================
// 7. Knowledge Vault / Blogs Logic
// ==========================================
window.blogArticles = [
  {
    id: 1,
    title: "The Art of Explaining Complex Ideas Simply",
    category: "Communication",
    readTime: 6,
    body: `
      <p>As software engineers, our day-to-day work is steeped in highly abstract concepts. However, our career success is often determined by our ability to convey these concepts to product managers, designers, executives, or junior developers. This article details three practical frameworks to master technical communication.</p>
      
      <h3>1. The Curse of Knowledge</h3>
      <p>The biggest obstacle to communication is the "curse of knowledge"—a cognitive bias where we unconsciously assume that our listener has the same background knowledge as we do. When explaining databases, we might jump straight to "B-Trees" or "Page cache hits" without establishing what a disk write is. To break this, we must build a bridge using <strong>Analogies</strong>.</p>
      
      <h3>2. The ELI5 Framework (Explain Like I'm 5)</h3>
      <p>The core rule of ELI5 is simple: <strong>Translate abstract algorithms into physical systems.</strong> Humans are evolved to understand physical movement, space, and gravity, not digital state transfer.</p>
      <blockquote><strong>Example:</strong> Explaining public-key encryption (SSL/TLS). Instead of explaining modular arithmetic on prime numbers, describe it as an open padlock sent in the mail. The sender closes the padlock, but only the owner holds the key to unlock it.</blockquote>
      
      <h3>3. The PREP Method for Quick Answers</h3>
      <p>When asked for a recommendation or system detail in standups, use the PREP method to prevent rambling:</p>
      <ul>
        <li><strong>Point:</strong> State your position immediately. ("We need to implement a Redis caching layer.")</li>
        <li><strong>Reason:</strong> Explain the logic. ("Our database read requests are slowing down the user feed profile loads.")</li>
        <li><strong>Example:</strong> Provide a concrete case. ("For example, our user dashboard takes 2.5 seconds to query profile history directly from Postgres.")</li>
        <li><strong>Point:</strong> Reiterate the conclusion. ("So, adding Redis will bring load times down to sub-10ms.")</li>
      </ul>
      
      <h3>4. The STAR Framework for Behavioral Stories</h3>
      <p>For interviews or architectural reviews, structure your stories around <strong>Situation, Task, Action, and Result</strong>. Focus 70% of your time on the <em>Action</em> (what decisions you made) and the <em>Result</em> (the measurable business metrics achieved, e.g., "reduced latency by 40%").</p>
    `
  },
  {
    id: 2,
    title: "Calculus in AI: The Mathematics of Neural Learning",
    category: "Math",
    readTime: 8,
    body: `
      <p>How does a computer "learn" from data? The magic of Machine Learning is entirely built on **Calculus**—specifically, rates of change and optimization.</p>
      
      <h3>1. Optimization and the Derivative</h3>
      <p>When we train a neural network, we write a <strong>Loss Function</strong> $L(W)$ that measures the network's prediction error. Our goal is to find the weights $W$ that minimize this error. In calculus, a minimum occurs where the rate of change is zero. The derivative $f'(x)$ gives us the slope of this function. By calculating the derivative, we know whether to increase or decrease the weights to slide down to the minimum error.</p>
      
      <h3>2. Gradient Descent</h3>
      <p>In high dimensions (where networks have billions of parameters), the derivative becomes the <strong>Gradient</strong> $\\nabla L$. The gradient is a vector pointing in the direction of steepest increase. By taking a step in the <em>opposite</em> direction, we perform Gradient Descent:
      $$W \\leftarrow W - \\eta \\nabla L$$
      Where $\\eta$ is the learning rate—the size of the step we take.</p>
      
      <h3>3. Differentiability and Activations</h3>
      <p>To compute these gradients, our activation functions must be <strong>differentiable</strong>. This is why historical step functions (which have a slope of 0 everywhere and are discontinuous at 0) were replaced by smooth curves like the **Sigmoid** or the piecewise-differentiable **ReLU** (Rectified Linear Unit), allowing optimization to propagate backwards.</p>
      
      <h3>4. The Chain Rule and Backpropagation</h3>
      <p>A neural network is a composition of functions: $f(g(h(x)))$. To find how the loss changes with respect to a weight in the first layer, we must apply the calculus **Chain Rule**:
      $$\\frac{\\partial L}{\\partial W_1} = \\frac{\\partial L}{\\partial y} \\cdot \\frac{\\partial y}{\\partial h} \\cdot \\frac{\\partial h}{\\partial W_1}$$
      This backward multiplication of partial derivatives is exactly what we call **Backpropagation**.</p>
    `
  },
  {
    id: 3,
    title: "Linear Algebra: The Coordinates of Machine Intelligence",
    category: "Math",
    readTime: 8,
    body: `
      <p>If Calculus is the engine of AI optimization, **Linear Algebra** is the canvas upon which AI is drawn. It defines how data is represented, transformed, and calculated.</p>
      
      <h3>1. Everything is a High-Dimensional Vector</h3>
      <p>In AI, all inputs must be converted into lists of numbers (vectors).
      - An image is represented as a matrix of pixel values.
      - A word (like "apple") is converted into a 768-dimensional **Word Embedding** vector.
      Similar words sit closer together in this vector space, allowing the AI to understand semantic relationships through vector math.</p>
      
      <h3>2. Matrix Multiplication as Space Transformation</h3>
      <p>When input vectors pass through a neural layer, they are multiplied by a weight matrix $W$:
      $$\\vec{y} = W\\vec{x} + \\vec{b}$$
      Geometrically, matrix multiplication scales, rotates, and projects the vector space. The neural network learns the exact coefficients of $W$ to stretch and bend the space so that different classes of data (e.g. spam vs. ham emails) are easily separable.</p>
      
      <h3>3. LU Decomposition</h3>
      <p>Solving systems of linear equations $A\\vec{x} = \\vec{b}$ is computationally intensive. Inverting a matrix ($A^{-1}$) is an $O(N^3)$ operation and numerically unstable. Instead, we use <strong>LU Decomposition</strong>, factoring $A$ into lower ($L$) and upper ($U$) triangular matrices. This splits the problem into two fast, successive steps (forward and back substitution), which is a key optimizer under the hood of scientific libraries.</p>
      
      <h3>4. Eigenvalues & Eigenvectors</h3>
      <p>Eigenvectors are directions in vector space that do not change orientation under a matrix transformation; they are only stretched by a factor of the eigenvalue $\\lambda$:
      $$A\\vec{v} = \\lambda\\vec{v}$$
      In AI, they are used in **Principal Component Analysis (PCA)** to reduce the dimension of embeddings while preserving variance, and in **Spectral Graph Theory** to partition large networks of interconnected tokens.</p>
    `
  },
  {
    id: 4,
    title: "Probability & Bayes: Reasoning Under Uncertainty",
    category: "Math",
    readTime: 7,
    body: `
      <p>In the real world, data is noisy and incomplete. An AI model can never be 100% certain. **Probability** is the mathematical framework that allows AI to measure and reason about uncertainty.</p>
      
      <h3>1. Random Variables and Distributions</h3>
      <p>In ML, we treat features as **Random Variables** that draw values from specific probability distributions.
      - **Normal (Gaussian) Distribution**: Used to model noise, weight initializations, and target errors.
      - **Poisson & Binomial Distributions**: Model discrete counts of clicks, page visits, or token counts.
      By predicting the parameters of these distributions (like the mean $\\mu$ and variance $\\sigma^2$), models generate realistic ranges rather than single guesses.</p>
      
      <h3>2. Conditional Probability</h3>
      <p>Conditional probability, $P(A|B)$, measures the likelihood of event $A$ occurring given that event $B$ has already occurred. This is the foundation of sequence generation: a language model calculates the probability of the next word given all previous words:
      $$P(W_n | W_{n-1}, W_{n-2}, ..., W_1)$$</p>
      
      <h3>3. Bayes' Theorem and Machine Learning</h3>
      <p>Bayes' Theorem updates our belief in a hypothesis ($A$) as new evidence ($B$) arrives:
      $$P(A|B) = \\frac{P(B|A) P(A)}{P(B)}$$
      In a **Naive Bayes Classifier**, we use this to compute the probability that a document belongs to a category (e.g. spam) given the words present in it. In **Bayesian Neural Networks**, weights are not single numbers; they are distributions of values representing the model's confidence in that connection.</p>
    `
  },
  {
    id: 5,
    title: "Architecting for Scale: System Design Patterns & Infrastructure",
    category: "System Design",
    readTime: 9,
    body: `
      <p>Building code that runs on your local machine is simple. Building systems that handle millions of requests concurrently without crashing is the core challenge of **System Design**. This article covers the foundations of distributed scalability.</p>
      
      <h3>1. The SOLID Principles for Maintainable Code</h3>
      <p>Before scaling servers, we must ensure our codebase doesn't rot. The **SOLID** principles decouple code modules, ensuring adding a new feature doesn't break old ones. For instance, the <em>Single Responsibility Principle</em> ensures a file does one job (like query databases), separating it from email notification triggers.</p>
      
      <h3>2. Load Balancers: The Traffic Cops</h3>
      <p>When a single server cannot handle incoming traffic, we scale horizontally by spinning up more instances. A <strong>Load Balancer (LB)</strong> sits in front, distributing requests across web servers using algorithms like Round Robin or Least Connections. LBs perform constant health checks; if a server crashes, the LB routes traffic away from it instantly, preventing downtime.</p>
      
      <h3>3. Caching: Speeding Up Reads</h3>
      <p>Database queries on disk are slow. If a million users request the same homepage feed, querying the database every time will crash the system. We insert a **Redis Cache** in memory.
      - **Cache Hit**: Data is fetched from memory in under 5ms.
      - **Cache Miss**: Fallback to database, retrieve it, store in cache for next time, and return.
      This decouples database operations from read traffic scaling.</p>
      
      <h3>4. Database Replication (Primary/Replica)</h3>
      <p>To scale write and read operations separately and provide redundancy:
      - **Primary Database**: Handles all data modifications (Writes, Updates, Deletes).
      - **Replica Databases**: Sync with the Primary asynchronously and serve all Read queries.
      Since web traffic is typically 90% reads and 10% writes, adding replicas directly scales system capacity.</p>
    `
  }
];

// Initialize and manage blogs index
function initBlogs() {
  const listContainer = document.getElementById('blogs-articles-list');
  if (!listContainer) return;
  listContainer.innerHTML = '';
  
  window.blogArticles.forEach((art, idx) => {
    const item = document.createElement('div');
    item.className = 'article-item';
    item.setAttribute('data-id', art.id);
    item.innerHTML = `
      <div class="art-category">${art.category}</div>
      <h4>${art.title}</h4>
      <div style="font-size: 0.7rem; color: var(--text-muted); margin-top: 0.25rem;">
        <i class="fa-regular fa-clock"></i> ${art.readTime} min read
      </div>
    `;
    ...
    item.addEventListener('click', () => {
      ...
      // On mobile, hide the index panel and show reader panel after click
      if (window.innerWidth <= 768) {
        document.getElementById('blogs-index-panel').classList.add('mobile-hidden');
        document.getElementById('article-reader-card').parentElement.classList.remove('mobile-hidden');
      }
    });
    listContainer.appendChild(item);
  });

  // Mobile Index Toggle Click
  const mobToggle = document.getElementById('mobile-blogs-toggle');
  if (mobToggle) {
    mobToggle.addEventListener('click', () => {
      const idxPanel = document.getElementById('blogs-index-panel');
      const readerPanel = document.getElementById('article-reader-card').parentElement;
      
      if (idxPanel.classList.contains('mobile-hidden')) {
        idxPanel.classList.remove('mobile-hidden');
        readerPanel.classList.add('mobile-hidden');
      } else {
        idxPanel.classList.add('mobile-hidden');
        readerPanel.classList.remove('mobile-hidden');
      }
    });
  }
  
  // Set default state on small viewports
  if (window.innerWidth <= 768) {
    document.getElementById('blogs-index-panel').classList.remove('mobile-hidden');
    document.getElementById('article-reader-card').parentElement.classList.add('mobile-hidden');
  }
}

function loadArticle(id) {
  const art = window.blogArticles.find(a => a.id === id);
  if (!art) return;
  
  document.getElementById('article-empty-state').classList.add('hidden');
  
  const contentState = document.getElementById('article-content-state');
  contentState.classList.remove('hidden');
  
  document.getElementById('article-title').innerText = art.title;
  document.getElementById('article-read-time').innerText = art.readTime;
  
  const catBadge = document.getElementById('article-category');
  catBadge.innerText = art.category;
  
  // Custom color badge depending on category
  if (art.category === 'Communication') {
    catBadge.style.background = 'rgba(0, 229, 255, 0.1)';
    catBadge.style.color = 'var(--accent-cyan)';
  } else if (art.category === 'Math') {
    catBadge.style.background = 'rgba(168, 85, 247, 0.1)';
    catBadge.style.color = 'var(--accent-purple)';
  } else {
    catBadge.style.background = 'rgba(245, 158, 11, 0.1)';
    catBadge.style.color = 'var(--accent-warning)';
  }
  
  const body = document.getElementById('article-body');
  body.innerHTML = art.body;
  
  // Trigger KaTeX dynamic compile for equations inside the article
