...
def blend_layers(original_rgb, color_layers, geometric_shapes=None, brightness=1.0, contrast=1.0):
    ...
    geometric_shapes: list of dicts with {'type', 'rect', 'color', 'opacity'}
    """
    ...
    # 2. Add geometric block overlays (for the modern Bauhaus/Mondrian/TikTok bypass style)
    if geometric_shapes:
        for shape in geometric_shapes:
            ...
            opacity = shape.get('opacity', 0.85)
            
            # Blend shape on top of the color canvas
            canvas_float[y1:y2, x1:x2] = (1.0 - opacity) * canvas_float[y1:y2, x1:x2] + opacity * geom_color
